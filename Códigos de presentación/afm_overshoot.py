...
plt.rc('axes', grid=True, labelsize=20, lw=1.8, titlesize=20, titlepad=12)
plt.rc('grid', c='d3d3d3', ls='--', lw=1.8)
plt.rc(['xtick', 'ytick'], direction='in', labelsize=18)
...
plt.subplots_adjust(top=0.94, bottom=0.08, left=0.055, right=0.98, hspace=0.40, wspace=0.30)

for i, ax in enumerate(axs.flat):
    ax.set_title('(%s) $K_P = %.1f$, $K_I = %.1f$, $K_D = %.1f$' % K[i])

axs[0,0].set_title(r"(a) Controlador ``perfecto''")

F = 99

for i, ax in enumerate(axs.flat):
    ax.plot([-10,410], [50,50], 'r--', lw=1.8)
    ax.plot(np.arange(px), D[i][F,:], lw=1.8)
    ax.set_xlabel('$x$ (px)')
    ax.set_ylabel('fuerza $F$ (nN)')
